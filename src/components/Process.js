  ...
  tick() {
   ...
   componentWillUnmount() {
    ...
    mostrarAlerta(s){
      swal({
        title: "Aviso",
        text: "¿Estas seguro que quieres detener este Proceso?",
        icon: "warning",
        buttons:["No","Si"]
      }).then(respuesta=>{
        if(respuesta){
          {this.stopProcess({s})}
          swal({text:"Proceso detenido con exito",
        icon:"success"})
        }
      })
    }
        ...
        return(
            ...
            <ul data-aos="fade-right" data-aos-duration="1000" class="list-group list-group-flush">
                   { this.state.processList.map(s => (  <li class="fondo list-group-item text-white"> {s}
                        ...
                        <button class="btn btn-danger" href="" onClick={(event) => {this.mostrarAlerta(s)}}>
